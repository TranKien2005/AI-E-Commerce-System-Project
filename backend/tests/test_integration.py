...
class TestBuyerJourney:
    ...
    def test_full_buyer_journey(self, client):
        ...
        reg = client.post(
            ...
            json={"email": email, "password": pwd, "full_name": "Journey Tester"},
        )
        ...
        assert (
            client.post(
                "/api/v1/cart/items", json={"product_id": pid, "quantity": 1}, headers=h
            ).status_code
            == 201
        )
        ...
        # Create order
        order = client.post(
            "/api/v1/orders", json={"shipping_address": "123 Journey St"}, headers=h
        )
        ...
        # Stock deducted
        assert (
            client.get(f"/api/v1/products/{pid}").json()["data"]["stock"]
            == initial_stock - 1
        )
...
class TestCancelRestoresStock:
    ...
    def test_cancel_restores_stock(self, client):
        ...
        if stock_before < 2:
            pytest.skip("Stock too low")

        client.post(
            "/api/v1/cart/items", json={"product_id": pid, "quantity": 1}, headers=h
        )
        order = client.post(
            "/api/v1/orders", json={"shipping_address": "Cancel Test"}, headers=h
        )
        assert order.status_code == 201
        oid = order.json()["data"]["order_id"]
...
class TestSellerViewsOrder:
    ...
    def test_seller_sees_buyer_order(self, client):
        ...
        order = client.post(
            "/api/v1/orders", json={"shipping_address": "Seller View Test"}, headers=h_b
        )
        if order.status_code != 201:
            pytest.skip("Could not create order")

        # Seller sees it
        seller_orders = client.get("/api/v1/seller/orders", headers=h_s).json()["data"][
            "items"
        ]
...
class TestProductLifecycle:
    ...
    def test_product_lifecycle(self, client):
        ...
        r = client.post(
            ...
            json={"name": "Lifecycle", "description": "d", "price": 100000, "stock": 5},
            headers=h,
        )
        ...
        search_ids = [
            p["id"] for p in client.get("/api/v1/products").json()["data"]["items"]
        ]
...
class TestPaymentIdempotency:
    ...
    def test_idempotent_payment(self, client):
        ...
        client.post(
            "/api/v1/cart/items", json={"product_id": p["id"], "quantity": 1}, headers=h
        )
        order = client.post(
            "/api/v1/orders", json={"shipping_address": "Idempotent Test"}, headers=h
        )
        if order.status_code != 201:
            pytest.skip("Could not create order")
        oid = order.json()["data"]["order_id"]
        ...
        r1 = client.post(
            f"/api/v1/payments/{oid}/pay", headers={**h, "X-Idempotency-Key": key}
        )
