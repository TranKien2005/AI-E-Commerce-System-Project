...
class TestProfile:
    ...
    def test_update_me(self, client):
        assert (
            client.put(
                "/api/v1/users/me", json={"full_name": "Demo Buyer"}, headers=bh(client)
            ).status_code
            == 200
        )
...
class TestCart:
    ...
    def test_update_cart_item(self, client):
        ...
        if not cart:
            pytest.skip("Empty cart")
        r = client.put(
            f"/api/v1/cart/items/{cart[0]['item_id']}",
            json={"quantity": 1},
            headers=bh(client),
        )
    ...
    def test_delete_cart_item_404(self, client):
        assert (
            client.delete("/api/v1/cart/items/999999", headers=bh(client)).status_code
            == 404
        )
...
class TestOrders:
    ...
    def test_filter_by_status(self, client):
        r = client.get(
            "/api/v1/orders/my", params={"status": "delivered"}, headers=bh(client)
        )
    ...
    def test_filter_by_date(self, client):
        r = client.get(
            "/api/v1/orders/my", params={"from_date": "2020-01-01"}, headers=bh(client)
        )
        assert r.status_code == 200

    def test_pagination(self, client):
        r = client.get(
            "/api/v1/orders/my", params={"page": 1, "page_size": 2}, headers=bh(client)
        )
        assert r.json()["data"]["meta"]["page_size"] == 2

    def test_order_detail(self, client):
        orders = client.get("/api/v1/orders/my", headers=bh(client)).json()["data"][
            "items"
        ]
        if not orders:
    ...
    def test_order_detail_has_items(self, client):
        ...
        if not orders:
            pytest.skip("No orders")
        d = client.get(f"/api/v1/orders/{orders[0]['id']}", headers=bh(client)).json()[
            "data"
        ]
    ...
    def test_order_404(self, client):
        assert (
            client.get("/api/v1/orders/999999", headers=bh(client)).status_code == 404
        )
    ...
    def test_cancel_delivered_fails(self, client):
        ...
        assert (
            client.post(
                f"/api/v1/orders/{orders[0]['id']}/cancel", headers=bh(client)
            ).status_code
            == 400
        )
...
class TestPayment:
    ...
    def test_payment_404(self, client):
        assert (
            client.get("/api/v1/payments/999999", headers=bh(client)).status_code == 404
        )
...
class TestReviews:
    def test_invalid_rating(self, client):
        ...
        r = client.post(
            "/api/v1/reviews",
            json={"product_id": products[0]["id"], "rating": 10, "comment": "bad"},
            headers=bh(client),
        )
    ...
    def test_review_no_purchase(self, client):
        r = client.post(
            "/api/v1/reviews",
            json={"product_id": 999999, "rating": 5, "comment": "x"},
            headers=bh(client),
        )
...
class TestNotifications:
    ...
    def test_mark_all_read(self, client):
        assert (
            client.patch(
                "/api/v1/notifications/read-all", headers=bh(client)
            ).status_code
            == 200
        )
...
class TestReports:
    def test_create_report(self, client):
        assert (
            client.post(
                "/api/v1/reports",
                json={"target_type": "product", "target_id": 1, "reason": "spam"},
                headers=bh(client),
            ).status_code
            == 201
        )
...
class TestSellerRequests:
    def test_my_request(self, client):
        assert (
            client.get("/api/v1/seller-requests/me", headers=bh(client)).status_code
            == 200
        )
...
class TestChat:
    def test_list_conversations(self, client):
        assert (
            client.get("/api/v1/chat/conversations", headers=bh(client)).status_code
            == 200
        )

    def test_full_chat_flow(self, client):
        users = client.get(
            "/api/v1/admin/users", params={"role": "seller"}, headers=ah(client)
        ).json()["data"]["items"]
        if not users:
            pytest.skip("No sellers")
        h = bh(client)
        # Create conversation
        r = client.post(
            "/api/v1/chat/conversations", json={"seller_id": users[0]["id"]}, headers=h
        )
        ...
        # List messages
        msgs = client.get(
            f"/api/v1/chat/conversations/{cid}/messages", headers=h
        ).json()["data"]["items"]
        assert len(msgs) >= 1
        # Mark read
        assert (
            client.patch(
                f"/api/v1/chat/conversations/{cid}/read", headers=h
            ).status_code
            == 200
        )
    ...
    def test_conversation_404(self, client):
        assert (
            client.get(
                "/api/v1/chat/conversations/999999/messages", headers=bh(client)
            ).status_code
            == 404
        )
