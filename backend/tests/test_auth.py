...
class TestRegister:
    def test_register_weak_password(self, client):
        r = client.post("/api/v1/auth/register", json={"email": "weak@t.com", "password": "abc", "full_name": "X"})
        assert r.status_code in (400, 422)

    def test_register_duplicate(self, client):
        r = client.post("/api/v1/auth/register", json={"email": B_EMAIL, "password": "Buyer@123", "full_name": "X"})
...
class TestLogin:
    def test_login_success(self, client):
        r = client.post("/api/v1/auth/login", json={"email": B_EMAIL, "password": B_PWD})
    ...
    def test_login_nonexistent_user(self, client):
        r = client.post("/api/v1/auth/login", json={"email": "noone@x.com", "password": "Abc@1234"})
        assert r.status_code == 401


class TestTokenLifecycle:
    def test_refresh_token(self, client):
        login = client.post("/api/v1/auth/login", json={"email": B_EMAIL, "password": B_PWD})
...
class TestPasswordReset:
    ...
    def test_verify_otp_wrong_code(self, client):
        ...
        r = client.post("/api/v1/auth/verify-otp", json={"email": B_EMAIL, "otp": "000000"})
    ...
    def test_verify_otp_correct_code(self, client):
        ...
        r = client.post("/api/v1/auth/verify-otp", json={"email": B_EMAIL, "otp": otp_code})
        assert r.status_code == 200

    def test_reset_password_bad_otp(self, client):
        r = client.post("/api/v1/auth/reset-password", json={"email": B_EMAIL, "otp": "", "new_password": "New@12345"})
        assert r.status_code == 400
    ...
    def test_reset_password_real_otp(self, client):
        ...
        client.post("/api/v1/auth/forgot-password", json={"email": B_EMAIL})
        otp_code = _otp_store.get(B_EMAIL, {}).get("code", "")
        r = client.post("/api/v1/auth/reset-password", json={"email": B_EMAIL, "otp": otp_code, "new_password": B_PWD})
        assert r.status_code == 200
