...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--import-reviews", action="store_true", help="Stream raw Amazon reviews for imported products and create buyer reviews/orders.")
    parser.add_argument("--limit", type=int, default=DEFAULT_PRODUCTS_PER_DOMAIN, help="Products per Amazon root domain.")
    parser.add_argument("--review-limit", type=int, default=DEFAULT_REVIEW_LIMIT_PER_DOMAIN, help="Maximum raw review rows to scan per Amazon root domain.")
    parser.add_argument("--dataset", default=DEFAULT_DATASET, help="Hugging Face dataset name.")
    parser.add_argument("--domain", action="append", dest="domains", help="Amazon root domain to import, e.g. Electronics. Can be repeated.")
    parser.add_argument("--dataset-config", help="Single Hugging Face metadata config. Kept for backwards compatibility.")
    ...
    parser.add_argument("--validate-videos", action="store_true", help="Probe product video URLs and store only reachable video URLs during product import.")
...
def selected_domains(args: argparse.Namespace) -> list[str]:
    if args.dataset_config:
        return [args.dataset_config.removeprefix("raw_meta_").removeprefix("raw_review_")]
...
def normalize_product(row: dict[str, Any], index: int, source_domain: str) -> ImportedProduct | None:
    title = clean_text(first_value(row, ["title", "name", "product_title"]))
    if not title:
        return None

    source_id = clean_text(first_value(row, ["parent_asin", "asin", "id"])) or stable_hash(f"{source_domain}:{title}")
...
def collect_products(args: argparse.Namespace, source_domain: str) -> list[ImportedProduct]:
...
def normalize_product_rows(rows: Any, limit: int, source_domain: str, seen_source_ids: set[str]) -> list[ImportedProduct]:
    products: list[ImportedProduct] = []
...
def import_products(db: Session, products: list[ImportedProduct], shops: list[Shop], mapper: CategoryMapper, validate_images: bool, validate_videos: bool) -> int:
    imported = 0
    skipped_images = 0
    skipped_videos = 0
    for index, imported_product in enumerate(products):
        ...
        if existing:
            ...
            db.execute(delete(ProductVideo).where(ProductVideo.product_id == product.id))
        else:
            product = Product(
                shop_id=shop.id,
        ...
        if validate_images:
            ...
            for image in images:
                if is_reachable_url(image.url, expected_prefix="image/"):
                    ...
                    skipped_images += 1
            images = valid_images
        images = sorted(images, key=lambda image: (image.variant.upper() != "MAIN", image.source_size != "hi_res", image.source_size != "large"))
        for image_index, image in enumerate(images):
            db.add(ProductImage(product_id=product.id, url=image.url[:500], is_primary=image_index == 0, variant=image.variant[:50], source_size=image.source_size[:20]))
        ...
        if validate_videos:
            ...
            videos = valid_videos
        for video in videos:
            db.add(ProductVideo(product_id=product.id, title=video.title[:255], url=video.url[:500], source_user_id=video.source_user_id[:255]))
        imported += 1
...
def normalize_review(row: dict[str, Any]) -> ImportedReview | None:
    ...
    source_review_id = stable_hash(f"{source_user_id}:{product_source_id}:{row.get('timestamp')}:{rating}:{title}:{text[:64]}")
...
def import_reviews_for_domain(db: Session, args: argparse.Namespace, source_domain: str) -> tuple[int, int, int]:
    product_by_source_id = {
        product.attributes.get("source_id"): product
        for product in db.scalars(select(Product).where(Product.attributes["source_domain"].as_string() == source_domain)).all()
    ...
    try:
        try:
            rows = stream_amazon_rows(args.dataset, review_config(source_domain), args.split, args.trust_remote_code)
            import_reviews, users, orders = import_review_rows(db, rows, args.review_limit, product_by_source_id)
        except Exception:
        ...
        raise RuntimeError(f"Could not stream Amazon raw reviews for {source_domain}.") from exc
    return imported_reviews, created_users, created_orders


def import_review_rows(db: Session, rows: Any, review_limit: int, product_by_source_id: dict[str, Product]) -> tuple[int, int, int]:
    imported_reviews = 0
    created_users = 0
    created_orders = 0
    for row in islice(rows, review_limit):
        ...
        if not review or review.product_source_id not in product_by_source_id:
            continue
        if db.scalar(select(Review).where(Review.source_review_id == review.source_review_id)):
            continue
...
def create_seed_order(db: Session, user: User, product: Product, review: ImportedReview) -> None:
    ...
    db.add(Payment(order_id=order.id, method="ONLINE", status="paid", transaction_id=f"seed-{review.source_review_id}", idempotency_key=f"seed-{review.source_review_id}"))
...
def parse_details(value: Any) -> dict[str, Any]:
    if isinstance(value, dict):
        return {clean_text(key): clean_text(val) for key, val in value.items() if clean_text(key) and clean_text(val)}
    if isinstance(value, str) and value.strip():
        try:
            parsed = json.loads(value)
            if isinstance(parsed, dict):
                return {clean_text(key): clean_text(val) for key, val in parsed.items() if clean_text(key) and clean_text(val)}
...
def normalize_details(details: dict[str, Any]) -> dict[str, str]:
    ...
    normalized_keys = {re.sub(r"[^a-z0-9]+", " ", key.lower()).strip(): clean_text(value) for key, value in details.items()}
    output: dict[str, str] = {}
...
def extract_images(row: dict[str, Any]) -> list[ImportedImage]:
    ...
    for key in ["image", "main_image", "large", "hi_res"]:
        ...
        if isinstance(value, str):
            images.extend(ImportedImage(url=url, variant="", source_size=key) for url in image_urls_from_text(value))
    seen = set()
    unique = []
    for image in images:
    ...
    if isinstance(value, dict):
        ...
        for source_size in ["hi_res", "large", "thumb", "main", "url"]:
            ...
            for image_url in image_urls_from_text(url):
                images.append(ImportedImage(url=image_url, variant=variant, source_size=source_size))
...
def extract_videos(row: dict[str, Any]) -> list[ImportedVideo]:
    ...
    for item in row.get("videos") or []:
        ...
        if url.startswith("http"):
            videos.append(ImportedVideo(title=clean_text(item.get("title")), url=url, source_user_id=clean_text(item.get("user_id"))))
    return videos
    ...
    try:
        ...
        if args.dry_run:
            for domain in domains:
                products = collect_products(args, domain) if args.import_products else []
                print(f"Dry run: collected {len(products)} products from {args.dataset}/{metadata_config(domain)}:{args.split}.")
                for product in products[:5]:
                    print(f"- {domain} | {product.title} | {product.price} | images={len(product.images)} | videos={len(product.videos)} | details={len(product.attributes.get('details', {}))}")
            return
        ...
        if args.import_products:
            for domain in domains:
                ...
                domain_imported = import_products(db, products, shops, mapper, args.validate_images, args.validate_videos)
                imported += domain_imported
                print(f"Imported domain {domain}: products={domain_imported}/{args.limit}")
        ...
        if args.import_reviews:
            for domain in domains:
                ...
                user_total += users
                order_total += orders
                print(f"Imported reviews for {domain}: reviews={reviews}, users={users}, orders={orders}")

        db.commit()
        print(f"Seed completed. Imported products={imported}, reviews={review_total}, users={user_total}, orders={order_total}.")
        print_counts(db, "Current counts")
    except Exception:
        db.rollback()
        raise
