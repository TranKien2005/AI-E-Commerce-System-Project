...
def update_shop(
    ...
    return seller_service.update_shop(
        db, current_user, payload.name, payload.description
    )
...
def create_product(
    ...
    return seller_service.create_product(
        db,
        current_user,
        payload.name,
        payload.description,
...
def update_product(
    ...
    return seller_service.update_product(
        db,
        current_user,
        id,
        payload.name,
...
def patch_product_image(
    ...
    return seller_service.set_product_image_primary(
        db, current_user, id, payload.is_primary
    )
...
def update_shipping(
    ...
    return seller_service.update_shipping(
        db, current_user, id, payload.shipping_status, payload.shipping_note
    )
...
def send_chat_message(
    ...
    return seller_service.send_chat_message(
...
def patch_chatbot_config(
    payload: ChatbotConfigIn,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_roles("seller", "admin")),
):
    return seller_service.patch_chatbot_config(
        db,
        current_user,
        payload.api_key,
        payload.prompt,
