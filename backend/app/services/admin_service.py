...
def patch_user(
    db: Session, admin_id: int, user_id: int, status: str | None, role: str | None
):
    ...
    if user_id == admin_id and (
        (role and role != "admin") or (status and status != "active")
    ):
...
def list_seller_requests(db: Session):
    items = db.scalars(
        ...
        .where(SellerRequest.status == "pending")
...
def patch_seller_request(
    db: Session, admin_id: int, request_id: int, action: str, reason: str
):
    ...
    if action == "approve":
        ...
        if user:
            ...
            if not existing_shop:
                shop = Shop(
                    owner_id=user.id, name=req.shop_name, description=req.description
                )
                db.add(shop)
            db.add(
                Notification(
                    user_id=user.id,
                    content=f"Yêu cầu trở thành người bán đã được duyệt. Cửa hàng '{req.shop_name}' đã được tạo.",
                    type="system",
                    channel="email",
                )
            )
            email_payload = (
                user.email,
                "Yêu cầu mở cửa hàng đã được duyệt",
                f"Chúc mừng! Cửa hàng <b>{req.shop_name}</b> đã được tạo thành công.",
            )
    else:
        db.add(
            Notification(
                user_id=req.user_id,
                content=f"Yêu cầu trở thành người bán đã bị từ chối. Lý do: {reason}",
                type="system",
                channel="email",
            )
        )
        ...
        if user:
            email_payload = (
                user.email,
                "Yêu cầu mở cửa hàng bị từ chối",
                f"Yêu cầu mở cửa hàng đã bị từ chối. Lý do: {reason}",
            )
    db.add(
        AuditLog(
            admin_id=admin_id,
            action="patch_seller_request",
            target_type="seller_request",
            target_id=request_id,
            description="Admin moderated seller request",
        )
    )
    db.commit()
    if email_payload:
...
def patch_report(db: Session, admin_id: int, report_id: int, status_value: str):
    ...
    db.add(
        AuditLog(
            ...
            target_type="report",
            target_id=report_id,
            description="Admin updated report",
        )
    )
    db.commit()
    return ok({})
