...
def _ensure_order_owned_by_shop(db: Session, shop: Shop | None, order_id: int):
    ...
    owns = db.scalar(
        ...
        .where(OrderItem.order_id == order_id, Product.shop_id == shop.id)
    )
    if not owns:
...
def add_product_image(
    db: Session, user: User, product_id: int, url: str, is_primary: bool
):
...
def _serialize_order_for_seller(
    db: Session, order: Order, shop: Shop | None = None
) -> dict:
    ...
    if shop:
        item_stmt = item_stmt.join(Product, Product.id == OrderItem.product_id).where(
...
def update_order_status(db: Session, user: User, order_id: int, status: str):
    ...
    db.add(
        Notification(
            user_id=order.user_id,
            content=f"Đơn hàng #{order.id} đã chuyển sang trạng thái: {status}",
            type="order",
            channel="email",
        )
    )
    ...
    account_events.publish_from_thread(
        order.user_id, {"type": "order_updated", "order_id": order.id}
    )
    buyer = db.get(User, order.user_id)
    if buyer:
        _send_email_after_commit(
            buyer.email,
            f"Đơn hàng #{order.id} - Cập nhật trạng thái",
            f"Đơn hàng <b>#{order.id}</b> đã chuyển sang: <b>{status}</b>",
        )
    return ok({})


def update_shipping(
    db: Session, user: User, order_id: int, shipping_status: str, shipping_note: str
):
    ...
    db.add(
        Notification(
            user_id=order.user_id,
            content=f"Vận chuyển đơn #{order.id} đã cập nhật: {shipping_status}",
            type="shipping",
            channel="system",
        )
    )
...
def patch_chatbot_config(
    db: Session, user: User, api_key: str, prompt: str, template: str, is_enabled: bool
):
    shop = _get_owner_shop(db, user)
    if not shop:
...
def get_stats(
    db: Session, user: User, from_date: str | None = None, to_date: str | None = None
):
    ...
    if from_date:
        try:
            order_ids_q = order_ids_q.where(
                Order.created_at >= datetime.fromisoformat(from_date)
            )
        except ValueError:
            pass
    if to_date:
        try:
            order_ids_q = order_ids_q.where(
                Order.created_at <= datetime.fromisoformat(to_date)
            )
        except ValueError:
            pass
    ...
    best_selling = [
        {"id": row[0], "name": row[1], "sold": int(row[2])}
        for row in db.execute(best_selling_q).all()
    ]

    return ok(
        {"revenue": revenue, "orders": len(order_ids), "best_selling": best_selling}
    )
...
def list_chat_conversations(db: Session, user: User):
    ...
    items = db.scalars(
        select(Conversation).where(
            Conversation.shop_id == shop.id, Conversation.seller_id == user.id
        )
    ).all()
...
def _ensure_seller_conversation(
    ...
    if (
        not conversation
        or not shop
        or conversation.shop_id != shop.id
        or conversation.seller_id != user.id
    ):
...
def get_chatbot_config(db: Session, user: User):
    ...
    masked = (
        cfg.api_key[:3] + "..." + cfg.api_key[-4:] if len(cfg.api_key) > 7 else "***"
    )
    return ok(
        {
            "api_key": masked,
