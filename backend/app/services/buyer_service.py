...
from app.models.entities import CartItem, Category, Notification, Order, OrderItem, Payment, Product, ProductImage, Report, Review, SellerRequest, Shop, User
...
def create_order(db: Session, user: User, shipping_address: str):
    ...
    for c in cart_items:
        ...
        if not p or p.deleted_at is not None:
            fail(400, "BAD_REQUEST", f"Sáº£n pháº©m #{c.product_id} khÃ´ng cÃ²n tá»“n táº¡i")
        if p.stock < c.quantity:
            fail(400, "BAD_REQUEST", f"Sáº£n pháº©m '{p.name}' khÃ´ng Ä‘á»§ tá»“n kho (cÃ²n {p.stock}, cáº§n {c.quantity})")
    ...
    db.add(Notification(user_id=user.id, content=f"ÄÆ¡n hÃ ng #{order.id} Ä‘Ã£ Ä‘Æ°á»£c táº¡o thÃ nh cÃ´ng", type="order", channel="email"))
    for seller_id in seller_ids:
        db.add(Notification(user_id=seller_id, content=f"Shop cá»§a báº¡n cÃ³ Ä‘Æ¡n hÃ ng má»›i #{order.id}", type="seller_order", channel="system"))
    db.commit()
    ...
    _send_email_after_commit(user.email, f"ÄÆ¡n hÃ ng #{order.id} Ä‘Ã£ Ä‘Æ°á»£c táº¡o", f"ÄÆ¡n hÃ ng <b>#{order.id}</b> Ä‘Ã£ Ä‘Æ°á»£c táº¡o thÃ nh cÃ´ng. Tá»•ng: {total:,.0f}Ä‘")
...
def pay_order(db: Session, user: User, order_id: int, x_idempotency_key: str | None):
    ...
    if result_status == "success":
        ...
        db.add(Notification(user_id=user.id, content=f"Thanh toÃ¡n Ä‘Æ¡n #{order_id} thÃ nh cÃ´ng", type="payment", channel="email"))
    else:
        order.payment_status = "failed"
        db.add(Notification(user_id=user.id, content=f"Thanh toÃ¡n Ä‘Æ¡n #{order_id} tháº¥t báº¡i, vui lÃ²ng thá»­ láº¡i", type="payment", channel="email"))
    db.commit()
    if result_status == "success":
        _send_email_after_commit(user.email, f"Thanh toÃ¡n Ä‘Æ¡n #{order_id} thÃ nh cÃ´ng", f"ÄÆ¡n hÃ ng <b>#{order_id}</b> Ä‘Ã£ Ä‘Æ°á»£c thanh toÃ¡n thÃ nh cÃ´ng.")
    else:
        _send_email_after_commit(user.email, f"Thanh toÃ¡n Ä‘Æ¡n #{order_id} tháº¥t báº¡i", f"Thanh toÃ¡n Ä‘Æ¡n hÃ ng <b>#{order_id}</b> tháº¥t báº¡i. Vui lÃ²ng thá»­ láº¡i.")
...
def create_review(db: Session, user: User, product_id: int, rating: int, comment: str, title: str = ""):
    ...
    if not order_ids:
        fail(403, "FORBIDDEN", "Chá»‰ Ä‘Æ°á»£c Ä‘Ã¡nh giÃ¡ sáº£n pháº©m Ä‘Ã£ mua trong Ä‘Æ¡n delivered")
...
def cancel_order(db: Session, user: User, order_id: int):
    ...
    if order.status not in {"pending", "processing"}:
        fail(400, "BAD_REQUEST", "KhÃ´ng thá»ƒ há»§y Ä‘Æ¡n á»Ÿ tráº¡ng thÃ¡i hiá»‡n táº¡i")
    ...
    for oi in order_items:
        ...
        if p:
            p.stock += oi.quantity
    db.add(Notification(user_id=user.id, content=f"ÄÆ¡n hÃ ng #{order_id} Ä‘Ã£ Ä‘Æ°á»£c há»§y", type="order", channel="email"))
    db.commit()
    account_events.publish_from_thread(user.id, {"type": "order_updated", "order_id": order.id})
    _send_email_after_commit(user.email, f"ÄÆ¡n hÃ ng #{order_id} Ä‘Ã£ há»§y", f"ÄÆ¡n hÃ ng <b>#{order_id}</b> Ä‘Ã£ Ä‘Æ°á»£c há»§y thÃ nh cÃ´ng.")
    return ok({})
...
def update_cart(db: Session, user: User, item_id: int, quantity: int):
    ...
    if p and p.stock < quantity:
        fail(400, "BAD_REQUEST", f"Sáº£n pháº©m khÃ´ng Ä‘á»§ tá»“n kho (cÃ²n {p.stock})")
